  ...
  class Blipper_Widget extends \WP_Widget {
    ...
    public static function bw_shortcode_blip_display( array $shortcode_attributes, string $content = '', string $shortcode = '' ): string {
      ...
      try {
        ...
        $new_cache_key = self::bw_generate_cache_key( $current_attributes );
        ...
        if ( false !== $post_id ) {
          ...
          $old_cache_key = self::bw_generate_cache_key( $old_attributes );
     ...
     */
    private static function bw_generate_cache_key( array $settings, ?string $the_blip_title = null, ?int $cache_expiry = null ): string {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
     ...
     * failure.
     */
    private static function bw_render_the_blip( array $user_attributes, bool $is_widget, false|string $widget_id = false, ?string $the_blip_title = null, ?array $widget_settings = null, ?string $content = null, ?string $cache_key = null ) {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
     ...
     */
    private static function bw_generate_blip( array $user_attributes, bool $is_widget, false|string $widget_id = false, ?array $widget_settings = null, ?string $content = null ): string|false {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
      ...
      if ( ( self::$client ?? null ) !== null ) {
        ...
        try {
          $cache_blip = $is_widget ? self::bw_should_cache_blip() : false;
        } catch ( \Exception $e ) {
          self::bw_display_error_msg( $e );
        } finally {
          error_log( 'caching generated blip ' . ( $is_widget ? var_export( $widget_id, true ) : 'shortcode' ) . ': ' . var_export( $cache_blip, true ) );
          if ( $cache_blip ) {
            try {
              $cache_key = self::bw_generate_cache_key( $user_attributes );
              error_log( 'cache key: ' . var_export( $cache_key, true ) );
              $result = self::bw_set_cache(
                cache_key: $cache_key,
                data_to_cache: $the_blip,
                is_widget: $is_widget,
                widget_id: $widget_id,
                cache_the_key: true
              );
              error_log( 'set cache of generated blip: ' . var_export( $result, true ) );
              // // If the blip is for a widget, connect the widget id with this cache key:
              // if ( $result && $is_widget && $widget_id ) {
              //   error_log( 'caching cache key of blip ' . var_export( $widget_id, true ) );
              //   $result = self::bw_cache_blip_cache_key(
              //     data_to_cache: $cache_key,
              //     widget_id: $widget_id
              //   );
              //   if ( !$result ) {
              //     $result = self::bw_cache_blip_cache_key( $cache_key_cache_key, $user_attributes );
              //   }
              //   error_log( 'cached cache key of cache key: ' . var_export( $result, true ) );
              // } else {
              //   error_log( 'not caching cache key of blip ' . var_export( $widget_id, true ) );
              // }
            } catch ( \Exception $e ) {
              self::bw_display_error_msg( $e );
            }
     ...
     */
    private static function bw_set_cache( string $cache_key, string $data_to_cache, bool $is_widget, ?bool $cache_the_key = null, ?string $widget_id = null ): bool {
      bw_log( 'method', __METHOD__ . '()' );
      bw_log( 'arguments', func_get_args() );
      ...
      try {
        if ( self::CACHE_EXPIRY ?? is_numeric( self::CACHE_EXPIRY ) ) {
          ...
          $transient = get_transient( $cache_key );
          // error_log( 'transient contents'  . ( false === $transient ? '' : ' (old)' ) . ': ' . var_export( get_transient( $cache_key ), true ) );
          $limit = str_starts_with( $cache_key, BW_PREFIX . BW_ID_BASE ) ? strlen( BW_PREFIX . BW_ID_BASE ) + 3 : strlen( BW_PREFIX ) + 5;
          // error_log( ( ( false === $transient ) ? 'writing' : 'overwriting' ) . ' transient ' . var_export( self::bw_get_truncated_cache_key( $cache_key, $limit ), true ) );
          // Have the old blip expire after any current blip:
          $expiry = self::CACHE_EXPIRY;//str_starts_with( $cache_key, BW_PREFIX . '-' . BW_ID_BASE ) ? self::CACHE_EXPIRY + HOUR_IN_SECONDS : self::CACHE_EXPIRY;
          // error_log( 'expiry time: ' . var_export( $expiry, true ) . ' (' . var_export( gettype( $expiry ), true ) . ')' );
          $result = set_transient( $cache_key, $data_to_cache, $expiry ) || get_transient( $cache_key ) === $transient;
          error_log( 'set transient ' . var_export( self::bw_get_truncated_cache_key( $cache_key, $limit ), true ) . ': ' . var_export( $result, true ) . ( ( !$result && get_transient( $cache_key ) === $transient ) ? ' – transient is unchanged' : ( $result ? ' – succeeded' : ' – failed' ) ) );
          error_log( 'transient contents: ' . var_export( get_transient( $cache_key ), true ) );
          // error_log( 'transient contents'  . ( false === $transient ? '' : ' (now)' ) . ': ' . var_export( get_transient( $cache_key ), true ) );
      ...
      try {
        ...
        error_log( 'result of set cache: ' . var_export( $result, true ) );
        error_log( 'cache the key: ' . var_export( $cache_the_key, true ) );
        $cache_the_key = $cache_the_key ?? throw new \Exception( 'Argument $cache_the_key not set' );
        if ( true === $result && true === $cache_the_key && !empty( $widget_id ) ) {
          self::bw_cache_blip_cache_key(
            widget_id: $widget_id,
            data_to_cache: $cache_key
          );
        } else {
          error_log( 'not caching cache key' );
        }
        // self::bw_display_cache_status( $cache_key, __METHOD__ . '()', $result );
        return $result;
      }
    }
    ...
    private static function bw_delete_cache( string $cache_key ): bool {
      ...
      if ( empty( $cache_key ) ) {
        error_log( 'cannot delete cache: cache key has no value' );
        return false;
      } else {
        $transient = get_transient( $cache_key );
        ...
        error_log( 'deleted cache key ' . var_export( $cache_key, true ) . ': ' . var_export( $result, true ) . ( false === $transient ? ' – cache not found' : ( get_transient( $cache_key ) ? ' – cache still exists' : ' – cache deleted' ) ) );
        // self::bw_display_cache_status( $cache_key, 'Deleted cache', $result );
        return $result;
      }
    }

    private static function bw_display_cache_status( string $cache_key, string $message, ?bool $status = null, ?int $cutoff = null ): void {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );

      $includes_data = null === $status ? false : true;
      bw_log(
        data_name: $message . ' ' . var_export( self::bw_get_truncated_cache_key( $cache_key, $cutoff ), true ),
        data: $status ?? '',
    ...
    private static function bw_validate_widget_settings( array $new_settings, array $old_settings ): array {
      ...
      // $old_cache_key = self::bw_generate_cache_key( $old_settings );
      // self::bw_display_cache_status( $old_cache_key, 'Old cache key:' );
      // // error_log( 'old cache key: ' . var_export( $old_cache_key, true ) );

      // Get the validated settings' cache key:
      $validated_cache_key = self::bw_generate_cache_key( $validated_settings );
      // self::bw_display_cache_status( $validated_cache_key, 'Validated settings\' cache key:' );
      ...
      // }
      // bw_log( 'Validated widget settings', $validated_settings );
    ...
    /**
      ...
      */
    private static function bw_get_blip( string $styled_title, array $user_attributes, bool $is_widget, ?array $widget_settings = null, ?string $content = null ) {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
      ...
      if ( $continue ) {
        return '<div class="blipper-widget">' . $styled_title . $the_blip . '</div>';
      } else {
        return '';
      }
    }
    ...
    private static function bw_get_blip_get_blipfoto_user_profile( array &$data ): bool {
      ...
      try {
        if ( null === self::$client ) {
          throw new \InvalidArgumentException( 'The Blipfoto client does not exist' );
        }
    ...
    private static function bw_get_styled_title( array $user_attributes, ?array $widget_settings = null ) {
      ...
      $styled_title = '';
      if ( array_key_exists( 'title', $user_attributes ) && !empty( $user_attributes['title'] ) ) {
        $the_blip_title = $user_attributes['title'];
        if ( !empty( $user_attributes['title'] ) ) {
          // error_log( 'plain title: ' . var_export( $the_blip_title, true ) );
    ...
    */
    ...
    public static function bw_widget_exists( string $widget_id, ?\WP_Customize_Manager $wp_customise = null ): BW_Widget_Status {
      ...
      throw new \BadMethodCallException( 'Do not use ' . __METHOD__ . '()' );
    ...
    public function bw_delete_widget_from_customiser( ?\WP_Customize_Manager $wp_customise = null ): void {
      bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'argument', bw_array_to_string( $wp_customise ) );
      // bw_log( 'filter/action', current_filter() );

      $inactive_widget_ids = self::bw_get_inactive_widget_ids();
      if ( in_array( $this->id, $inactive_widget_ids ) ) {
        error_log( var_export( $this->id, true ) . ' is inactive; attempting to delete cache' );

        $cache_key_cache_key = self::bw_generate_cache_key_cache_key( $this->id );
        $cached_cache_key = self::bw_get_cache( $cache_key_cache_key );
        // $cached_blip = self::bw_get_cache($cached_blip_key );

        $settings = [];
        $result = $this->bw_get_widget_settings( $this->id, BW_ID_BASE, $settings );
        $settings_cache_key = $result ? self::bw_generate_cache_key( $settings ) : null;
        if ( empty( $cached_cache_key ) ) {
          error_log( 'widget ' . var_export( $this->id, true ) . ' has no cache key cached' );
          ...
          if ( $result ) {
            ...
            if ( $settings_cache_key === $cached_cache_key ) {
              ...
              error_log( 'cache key from settings: ' . var_export( $settings_cache_key, true ) );
              error_log( 'cache key from cache: ' . var_export( $cached_cache_key, true ) );
        ...
        error_log( var_export( $this->id, true ) . ' is active; not deleting cache' );
      }
    }
    ...
    /**
     * Gets the Blipper Widgets that are on an active sidebar.
     *
     * @since 1.2.6
     * @author pandammonium
     *
     * @return array An array containing the IDs of any active Blipper
     * Widgets. If there aren't any, the array will be empty.
     */
    public static function bw_get_active_widget_ids(): array {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
      ...
      foreach ( $sidebars_widgets as $sidebar => $widget_ids ) {
        // error_log( 'widgets on sidebar ' . var_export( $sidebar, true ) . ': ' . var_export( $widget_ids, true ) );
        // error_log( 'sidebar: ' . var_export( $sidebar, true ) );
        if ( 'wp_inactive_widgets' === $sidebar ) {
          ...
          foreach ( $widget_ids as $widget_id ) {
            if ( str_starts_with( $widget_id, BW_ID_BASE ) ) {
              // error_log( 'active widget id: ' . var_export( $widget_id, true ) );
              $active_widget_ids[] = $widget_id;
            }
          }
        }
      }
    ...
    /**
     * Gets the Blipper Widgets that are in Appearance > Widgets > Inactive
     * Widgets.
     *
     * @since 1.2.6
     * @author pandammonium
     *
     * @return array An array containing the IDs of any inactive Blipper
     * Widgets. If there aren't any, the array will be empty.
     */
    public static function bw_get_inactive_widget_ids(): array {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
      ...
      if ( isset( $sidebars_widgets['wp_inactive_widgets'] ) ) {
        foreach ( $sidebars_widgets['wp_inactive_widgets'] as $widget_id ) {
          if ( str_starts_with( $widget_id, BW_ID_BASE ) ) {
            // error_log( 'inactive widget id: ' . var_export( $widget_id, true ) );
            $inactive_widget_ids[] = $widget_id;
          }
        }
      }
      error_log( 'inactive widget ids: ' . var_export( $inactive_widget_ids, true ) );
      return $inactive_widget_ids;
    }
      ...
      if ( str_starts_with( haystack: $widget_id, needle: $id_base ) ) {
        ...
        if ( false === $result ) {
          ...
          if ( empty( $widget_settings ) ) {
            ...
            $cache_key = self::bw_generate_cache_key( $widget_settings );
    ...
    public function bw_on_customise_save_after( \WP_Customize_Manager $wp_customise ) {
      ...
      // bw_log( 'arguments', func_get_args() );
      bw_log( 'filter/action', current_filter() );
      bw_log( 'THIS WIDGET ID', $this->id );

      $this->bw_delete_widget_from_customiser( $wp_customise );
      $this->bw_cache_blip_in_customiser(
        wp_customise: $wp_customise,
        cache_the_key: true
      );
    }
    ...
    private function bw_save_customiser_widget( \WP_Customize_Manager $wp_customise ) {
      ...
      error_log( 'widget: ' . var_export( $this->id ) );
    ...
    public function bw_cache_blip_in_customiser( ?\WP_Customize_Manager $wp_customise = null, bool $cache_the_key = false ) {
      bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
      // bw_log( 'filter/action', current_filter() );
      ...
      if ( self::bw_should_cache_blip() ) {
        $client_ok = empty( self::$client ) ? self::bw_create_blipfoto_client() : true;
        if ( $client_ok ) {
         $active_widget_ids = self::bw_get_active_widget_ids();
          error_log( 'active widget ids: ' . var_export( $active_widget_ids, true ) );
          foreach ( $active_widget_ids as $widget_id ) {
            error_log( var_export( $widget_id, true ) . ' is active; attempting to cache' );
            $settings = [];
            $result = $this->bw_get_widget_settings( $widget_id, BW_ID_BASE, $settings );
            if ( false === $result ) {
              error_log( 'failed to get widget settings: ' , var_export( $settings, true ) );
            } else {
              $styled_title = self::bw_get_styled_title(
              ...
              error_log( 'cache key from settings: ' . var_export( $cache_key, true ) );
              $result = self::bw_set_cache(
                cache_key: $cache_key,
                data_to_cache: $the_blip,
                is_widget: true,
                widget_id: $widget_id,
                cache_the_key: $cache_the_key // cache the key only after deleting the current one
              );
              self::bw_display_cache_status( $cache_key, 'Set cache on customiser-save', $result );
              if ( $result ) {
                // Delete the cache stored in the cache key cache key, then cache the new cache key:
                $deleted = self::bw_delete_cached_blip( $widget_id, $cache_key, true );
                if ( $deleted ) {
                  $result = self::bw_cache_blip_cache_key(
                    widget_id: $widget_id,
                    data_to_cache: $cache_key
    ...
    /**
     * Deletes the cached blip.
     *
     * This includes the cached blip stored under the cache key and the cache
     * key stored under the cache key cache key.
     *
     * @since 1.2.6
     * @author pandammonium
     *
     * @param string $widget_id The identifier of the widget whose blip is to
     * be deleted.
     * @param string cache_key The cache key that the blip is cached under.
     * @param bool $delete_only_if_different If true, only delete the cache
     * key cache key and the cache key if they have different values. If
     * false, delete them all!
     * @return bool True if everything was deleted successfully; false on
     * failure.
     */
    private static function bw_delete_cached_blip( string $widget_id, string $cache_key, bool $delete_only_if_different = false ): bool {
      bw_log( 'method', __METHOD__ . '()' );
      bw_log( 'arguments', func_get_args() );
      // bw_log( 'filter/action', current_filter() );
      ...
      $cached_key = self::bw_get_cache( $cache_key_cache_key );
      error_log( 'old cache key: ' . var_export( $cached_key, true ) );
      error_log( 'new cache key: ' . var_export( $cache_key, true ) );
      $delete = $delete_only_if_different ? $cached_key !== $cache_key : true;
      if ( $delete ) {
        $deleted_cache_key = self::bw_delete_cache( $cached_key );
        $deleted_cache_key_cache_key = self::bw_delete_cache( $cache_key_cache_key );
        $deleted = $deleted_cache_key && $deleted_cache_key_cache_key;
      } else {
        error_log( 'did not delete old and new cache keys' );
      }
      return $deleted;
    }
    ...
    private static function bw_generate_cache_key_cache_key( $widget_id ): string {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );
      // bw_log( 'filter/action', current_filter() );

      $cache_key_cache_key = BW_PREFIX . $widget_id . BW_CACHE_KEY_CACHE_KEY_SUFFIX;
      bw_log( 'generated cache key cache key', $cache_key_cache_key );
      return $cache_key_cache_key;
    }
    ...
    private function bw_delete_old_widget_customiser_cache( \WP_Customize_Manager $wp_customise ) {
      ...
      foreach ( $settings as $setting_id => $setting ) {
        // error_log( 'customiser setting id: ' . bw_array_to_string( $setting_id ) );
        if ( str_starts_with( $setting_id, 'widget_' . BW_ID_BASE ) ) {
          // error_log( 'setting id: ' . var_export( $setting_id, true ) );
          // error_log( 'class: ' . var_export( get_class( $setting ), true ) );
          // error_log( 'id data: ' . var_export( $setting->id_data(), true ) );
          // The keys give the number in the widget id string, for example blipper_widget-3. Use it and the id base to get the widget id:
          ...
          foreach ( $keys as $key_id => $instance_key ) {
            ...
            // error_log( 'widget id: ' . var_export( $widget_id, true ) );
            $cache_key_cache_key = bw_generate_cache_key_cache_key( $this->id );
            // error_log( 'cache-key cache key: ' . var_export( $cache_key_cache_key, true ) );
            $blip_cache_key = get_transient( $cache_key_cache_key );
            // error_log( 'stored cache key: ' . var_export( self::bw_get_truncated_cache_key( $blip_cache_key ), true ) );
            if ( false === $blip_cache_key ) {
              // self::bw_display_cache_status( $cache_key_cache_key, 'Did not find old blip cache key in' );
            } else {
              // self::bw_display_cache_status( $blip_cache_key, 'Found old blip cache key in' . var_export( $cache_key_cache_key, true ) );
              $current_settings = [];
              $result = $this->bw_get_widget_settings( $widget_id, BW_ID_BASE, $current_settings );
                // error_log( 'got current settings: ' . var_export( $result, true ) );
              if ( false === $result ) {
              } else {
                $current_cache_key = self::bw_generate_cache_key( $current_settings );
                // error_log( 'current cache key: ' . var_export( self::bw_get_truncated_cache_key( $current_cache_key ), true ) );
                if ( $current_settings === $blip_cache_key ) {
                  // error_log( 'new cache key equals old cache key' );
                } else {
                  $result = self::bw_get_cache( $blip_cache_key );
                  // error_log( 'found old blip cached as: ' . var_export( self::bw_get_truncated_cache_key( $blip_cache_key ), true ) . ': ' . var_export( $result, true ) );
                  $deleted = false === $result ? false : self::bw_delete_cache( $blip_cache_key );
                }
              }
            }
            // error_log( 'deleted old blip cache ' . var_export( self::bw_get_truncated_cache_key( $blip_cache_key ), true ) . ': ' . var_export( $deleted, true ) );
            // self::bw_display_cache_status( $blip_cache_key, 'Deleted old blip cache key', $deleted );
          }
          // error_log( 'id data - base: ' . var_export( $setting->id_data()['base'], true ) . PHP_EOL );
        }
      }
    }
    ...
    public function bw_capture_old_widget_settings( array $new_settings, array $old_settings ): array {
      ...
      try {
        ...
        $old_settings_extracted = [];

        foreach ( $new_settings as $new_key => $new_value ) {
          if ( BW_ID_BASE . '-' . $new_key === $this->id ) {
            $new_settings_extracted = $new_value;
            break;
          }
        }
        foreach ( $old_settings as $old_key => $old_value ) {
          if ( BW_ID_BASE . '-' . $old_key === $this->id ) {
            $old_settings_extracted = $old_value;
            break;
          }
        }

        if ( self::bw_compare_old_and_new_attributes( $old_settings_extracted, $new_settings_extracted, true ) ) {

          error_log( 'new settings extracted: ' . var_export( $new_settings_extracted, true ) );
          error_log( 'old settings extracted: ' . var_export( $old_settings_extracted, true ) );

          $result = self::bw_cache_blip_cache_key(
            settings: $old_settings_extracted,
            widget_id: $this->id
          );
          if ( !$result ) {
            throw new \Exception( 'Cache key of blip not cached.' );
          } else {
            error_log( 'cached old settings cache key' );
          }
        } else {
          error_log( 'old settings === new settings' );
        }
      } catch ( \Exception $e ) {
        self::bw_display_error_msg( $e );
      }
    ...
    private static function bw_should_cache_blip() {
      // bw_log( 'method', __METHOD__ . '()' );
      // bw_log( 'arguments', func_get_args() );

      $current_filter = current_filter();
      // error_log( 'current filter: ' . var_export( current_filter(), true ) );

      $is_customise_preview = is_customize_preview();
      // error_log( 'is customise preview: ' . var_export( is_customize_preview(), true ) );

      $should_cache_blip = 'customize_save_after' === $current_filter || !$is_customise_preview;
      // error_log( 'should cache blip: ' . var_export( $should_cache_blip, true ) );

      return $should_cache_blip;
    }

    /**
     * Caches the cache key used for the old settings.
     *
     * @return bool True if the blip cache key was cached; otherwise false;
     */
    private static function bw_cache_blip_cache_key( string $widget_id, ?array $settings = null, ?string $data_to_cache = null ): bool {
      ...
      try {
        if ( ( null === $settings && null === $data_to_cache ) ||
             ( null !== $settings && null !== $data_to_cache ) ) {
          throw new \InvalidArgumentException('Either the widget settings or the ready-to-cache data must be provided, but not both, and not neither.');
        }

        if ( self::bw_should_cache_blip() ) {
          // Generate the data for the new cache:
          $cache_key = BW_PREFIX . $widget_id . BW_CACHE_KEY_CACHE_KEY_SUFFIX;
          $data_to_cache = $data_to_cache ?? self::bw_generate_cache_key( $settings );
          error_log( 'attempting to cache ' . var_export( $cache_key, true ) . ': ' . var_export( $data_to_cache, true ) );
          // $set_cache = true;
          // Delete the blip whose cache key is stored in the given cache key, if it exists:
          $cache_key_for_blip_to_delete = get_transient( $cache_key );
          error_log( 'cache key ' . var_export( $cache_key, true ) . ' exists: ' . var_export( $cache_key_for_blip_to_delete, true ) );
          if ( $cache_key_for_blip_to_delete ) {
          //   if ( $cache_key_for_blip_to_delete !== $data_to_cache ) {
              $deleted = self::bw_delete_cache( cache_key_for_blip_to_delete );
              error_log( 'deleted old blip stored in ' . var_export( $cache_key, true ) . ' (' . self::bw_get_truncated_cache_key( $cache_key_for_blip_to_delete ) . ')' );
          //   } else {
          //     error_log( 'cache key for blip to delete is the same as the data to cache:' );
          //     error_log( var_export( $cache_key_for_blip_to_delete, true ) );
          //     error_log( var_export( $data_to_cache, true ) );
          //     $set_cache = false;
          //   }
          // } else {
          //   error_log( 'no cached blip to delete' );
          }

          // self::bw_display_cache_status( $data_to_cache, 'Cache ' . var_export( $cache_key, true ) . ' will contain' );
          // // error_log( 'going to cache this old cache key: ' . var_export( $data_to_cache, true ) . ' under cache key ' . var_export( $cache_key, true ) );

          // $cached_data_to_delete = get_transient( $cache_key );
          // $old_cache_exists = false !== get_transient( $cached_data_to_delete );
          // if ( $old_cache_exists ) {
          //   $result = delete_transient( $cached_data_to_delete );
          //   error_log( 'deleted existing cache ' .  var_export( self::bw_get_truncated_cache_key( $cache_key ), true ) . ': ' . var_export( $result, true ) );
          // } else {
          //   error_log( 'cache ' .  var_export( self::bw_get_truncated_cache_key( $cache_key, cut_off: strlen( $cache_key ) ), true ) . ' doesn\'t exist' );
          // }
          // if ( $set_cache ) {
          $result = self::bw_set_cache(
            cache_key: $cache_key,
          ...
          error_log( 'set cache ' . var_export( $cache_key, true ) . ': ' . var_export( $result, true ) );
          // } else {
          //   $deleted = self::bw_delete_cache( $cache_key );
          //   error_log( 'deleted cache ' . var_export( $cache_key, true ) . ': ' . var_export( $deleted, true ) );
          // }
        } else {
          error_log( 'cache key should not be cached' );
        ...
        // self::bw_display_cache_status( $data_to_cache, 'Cache blip cache key ' . var_export( $cache_key, true ), $result );
        return $result;
      }
    }
